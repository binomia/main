...
export class AccountController {
    static accounts = async (_: unknown, {page, pageSize}: {
        page: number,
        pageSize: number
    }, _context: any, {fieldNodes}: { fieldNodes: any }) => {
        try {
    ...
    static account = async (_: unknown, ___: unknown, {req, tracer}: { req: any, tracer: Tracer }, {fieldNodes}: {
        fieldNodes: any
    }) => {
    ...
    static accountPermissions = async (_: unknown, ___: unknown, {req}: { __: any, tracer: any, req: any }, {fieldNodes}: { fieldNodes: any }) => {
        try {
            const session = await checkForProtectedRequests(req);
    ...
    static updateAccountPermissions = async (_: unknown, {data}: { data: any }, {req}: {
        tracer: any,
        req: any
    }, {fieldNodes}: { fieldNodes: any }) => {
        try {
            const session = await checkForProtectedRequests(req);
            ...
            return await account.update(accountPermissions)
    ...
    static accountLimit = async (_: unknown, ___: unknown, {req}: { req: any }) => {
        try {
            const session = await checkForProtectedRequests(req);
