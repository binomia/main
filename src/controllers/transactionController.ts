...
export class TransactionsController {
    ...
    static createTransaction = async (_: unknown, { message }: { message: string }, { req, tracer }: { req: any, metrics: PrometheusMetrics, tracer: Tracer }) => {
        ...
        try {
            ...
            const pendingBalance = Number(account.pendingBalance + validatedData.amount)
            await AccountModel.update({
                pendingBalance: pendingBalance,
                balance: account.balance - pendingBalance
            }, {
            ...
            const ledgerData = {
                sender: {
                    ...
                    notes: "Transaction created and added to the queue server",
                    beforeBalance: user.account.balance,
                    afterBalance: user.account.balance - validatedData.amount || 0.00,
                ...
                receiver: {
                    ...
                    afterBalance: receiverAccount.dataValues.balance + validatedData.amount || 0.00,
                    latitude: validatedData.location.latitude,
                    longitude: validatedData.location.longitude
                }
            }
